...
function App() {
  ...
  const [initialStickerSize, setInitialStickerSize] = useState({
    width: 0,
    height: 0,
  });
  ...
  const handleTouchStart = useCallback(
    (sticker: StickerItem, event: React.TouchEvent) => {
    ...
    (event: React.TouchEvent) => {
      ...
      if (touchList.length === 2 && initialDistance > 0) {
        ...
        const newWidth = Math.max(
          20,
          Math.min(300, initialStickerSize.width * scale)
        );
        const newHeight = Math.max(
          20,
          Math.min(300, initialStickerSize.height * scale)
        );
        ...
        const newX = Math.max(
          0,
          Math.min(canvasSize.width - 50, coords.x - dragOffset.x)
        );
        const newY = Math.max(
          0,
          Math.min(canvasSize.height - 50, coords.y - dragOffset.y)
        );
  ...
  const handleResizeMouseDown = useCallback(
    (sticker: StickerItem, handle: string, event: React.MouseEvent) => {
      event.preventDefault();
      event.stopPropagation();
                        ...
                        return (
                          <div className="space-y-3">
                            <div className="grid grid-cols-2 gap-2">
                              <div>
                                <Label className="text-xs">
                                  Šírka: {Math.round(sticker.width)}
                                </Label>
                                <Slider
                                  value={[sticker.width]}
                                  onValueChange={([value]) =>
                              ...
                              <div>
                                <Label className="text-xs">
                                  Výška: {Math.round(sticker.height)}
                                </Label>
                                <Slider
                            ...
                            <div className="text-xs text-gray-500">
                              💡 Tip: Na mobile použite pinch gesture pre
                              resize, na desktope ťahajte za modré body
                            </div>
                          </div>
                        );
                      })()}
